 ...
 <?php
    ...
    if ($secureHash == $vnp_SecureHash) {
        if ($_GET['vnp_ResponseCode'] == '00') {
            ...
            $conn->query("UPDATE dondathang SET tt_thanhtoan = 'Đã thanh toán' WHERE ma_ddh = '$order_id'");
            unset($_SESSION['cart']);
        } else {
            $status_msg = "Giao dịch thất bại hoặc bị hủy";
            $icon = "bi-x-circle-fill text-danger";
            $conn->query("UPDATE dondathang SET trangthai = 'Đã hủy' WHERE ma_ddh = '$order_id'");
        }
    } else {
